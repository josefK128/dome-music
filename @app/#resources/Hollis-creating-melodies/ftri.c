...
void walk( int i, int j, int nstep )
{
  ...
  if( nstep<walklen ){
    if(i<nc){walkstr[nstep]='r'; walk(i+1,j,nstep+1);}
    if(i>0){walkstr[nstep]='l'; walk(i-1,j,nstep+1);}
    if(j<nr){walkstr[nstep]='u'; walk(i,j+1,nstep+1);}
    if(j>0){walkstr[nstep]='d'; walk(i,j-1,nstep+1);}
    if(j<nr && i<nc){walkstr[nstep]='v'; walk(i+1,j+1,nstep+1);}
    if(j>0 && i>0){walkstr[nstep]='e'; walk(i-1,j-1,nstep+1);}}
}
...
{
  if( argc < 8 ){
    printf("Usage: ftri nc nr x y a b n\n");
    printf("  Generates paths in a finite triangular lattice from (x,y) to (a,b).\n");
